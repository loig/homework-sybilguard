...
trusted authority responsible of ensuring a one-to-one correspondance
...
garantees that, by randomly assigning the replicas, the probability of
...
explain how it is possible to deal with dynamic networks. Unlike a
traditional paper and in order to emphasize the practical value of
this system, the experimental results are presented along the text, to
confirm the author's statements.
...
among all these groups, at most $g$ contains Sybil nodes. 
Moreover SybilGuard bounds the size of each Sybil group. Let us denote
$w$ the length of random routes.  Given an edge $e$, we deduce from
the \emph{back-traceability property} of random routes -- going
...
verified will accept at most $w$ nodes by group.
SybilGuard also garantees that an honest node will accept other honest
...
These tables are simply built by propagation along neighbours.
...
node with 10 neighbours will have to send 2.56MB of data.
...
paper, the estimated length is optimal, with high probability.
...
nodes, and when a node propagates its tables to its neighbours, for
...
maintainance can take days to complete, without impacting the
